...
# Debug flag: when True, prints extra logs and keeps additional safety options
DEBUG_EXPORTER: bool = False
                ...
                if segment.material_data and hasattr(segment.material_data, 'remote_url'):
                    ...
                    if video_url:
                        ...
                        if video_filter:
                            ...
                            # Split per-segment stream for safe reuse in overlay and transition branches
                            ...
                            overlay_opts = ":shortest=1:eof_action=pass" if DEBUG_EXPORTER else ""
                            filter_parts.append(
                            ...
                            if last_tuple is not None:
                                ...
                                # Validate supported transitions (Pull_in, Black_Fade/Dissolve, Pull_out)
                                ...
                                if has_transition and not (is_pull_in or is_pull_out or enum_norm in ('black_fade', 'dissolve', 'fade')):
                                    raise ValueError(
                                        f"Unsupported transition for custom exporter: raw='{trans_name_str}' normalized='{trans_name_norm}' "
                                        f"resolved='{enum_name}'. Supported: Pull in, Pull out, Black_Fade/Dissolve."
                                    )
                                ...
                                if (is_pull_in or enum_norm in ('black_fade', 'dissolve', 'fade') or is_pull_out) and joins_cleanly and isinstance(trans_dur_sec, (int, float)) and trans_dur_sec > 0:
                                    ...
                                    # Debug: shave one frame to avoid endpoint rounding producing empty trims
                                    d_eff = d
                                    if DEBUG_EXPORTER:
                                    ...
                                    if (d_eff if DEBUG_EXPORTER else d) > 1e-3:
                                        ...
                                        d_used = d_eff if DEBUG_EXPORTER else d
                                        ...
                                        # Use split branches reserved for transitions
                                        prev_src = f"v{prev_loop_index}_xf"
                                        curr_src = f"v{i}_xf"
                                        ...
                                        xfade_type = 'fade' if enum_norm in ('black_fade', 'dissolve', 'fade') else ('zoomin' if is_pull_in else 'zoomout')
                                        filter_parts.append(f"[{A_full_cfr}][{B_full_cfr}]xfade=transition={xfade_type}:duration={(d_used if DEBUG_EXPORTER else d)}:offset=0[{trans_label}]")
                                        print(
                                            f"[XFADE] Built full-frame xfade type='{xfade_type}' between idx {prev_loop_index}->{i} over d={(d_used if DEBUG_EXPORTER else d):.3f}s"
                                        )

                                        # Use the full-frame xfade directly, replacing base during window
                                        ...
                                        overlay_opts2 = ":shortest=1:eof_action=pass" if DEBUG_EXPORTER else ""
                                        filter_parts.append(
